...
from utils.datasets1 import IMG_FORMATS, VID_FORMATS, LoadImages, LoadStreams
...
def run(
    ...
    # dataset1 = Image.open(io.BytesIO(source))
    # dataset2 = dataset1.load()
...
def region_detection(source_img, model_type):
    """ Region Detection """
    region_list, class_info = [], []
    try:
        if model_type == "visa":
            model_name = "visa.pt"
            class_info = ["visa_no", "doi", "doe", "poi", "type", "country_code", "visa_entry", "mrz_code"]
        elif model_type == 'local_id':
            model_name = 'local_id.pt'
            class_info = ['screen']
        else:
            model_name = "passport.pt"
            class_info = ["type", "country_code", "passport_no", "surname", "first_name", "nationality",
                          "sex", "dob", "pob", "poi", "doi", "doe", "mrz_code", "person"]
        weights = Path(os.getcwd() + "/yolo/" + model_name)
