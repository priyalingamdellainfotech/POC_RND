...
def inference_annotations(
    ...
    lw = max(round(sum(orig_image.shape) / 1 * 0.003), 2)  # Line width.
    ...
    for j, box in enumerate(draw_boxes):
        ...
        validation_data.append([f'{class_name} {str(round(scores[j], 2))}', p1[0], p1[1], p2[0], p2[1]])
        if args['track']:
        ...
        if not args['no_labels'] and scores[j] > 0.7:
