...
extension CameraFeedManager: AVCaptureVideoDataOutputSampleBufferDelegate {
  ...
  func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
    ...
    let pixelBuffer: CVPixelBuffer? = CMSampleBufferGetImageBuffer(sampleBuffer)
      ...
      // Combine or overlay images
      let combinedImage = combineImages(screenImage, with: uiImage)
      
      print(combinedImage.size)
    ...
    // Delegates the pixel buffer to the ViewController.
      delegate?.didOutput(pixelBuffer: imagePixelBuffer, sampleBuffer:sampleBuffer,originalImg: combinedImage)
  }
}

func combineImages(_ backgroundImage: UIImage, with overlayImage: UIImage) -> UIImage {
    UIGraphicsBeginImageContextWithOptions(backgroundImage.size, false, 0.0)
    backgroundImage.draw(in: CGRect(origin: .zero, size: backgroundImage.size))
    overlayImage.draw(in: CGRect(origin: .zero, size: backgroundImage.size))
    let combinedImage = UIGraphicsGetImageFromCurrentImageContext()
    UIGraphicsEndImageContext()
    return combinedImage ?? backgroundImage
}
